...
/**********************/
        ...
        FirstPoint = 10,               //начало отсчета для курсора(START_GAME)
        LastPoint = 12                 //конец отсчета для курсора(EXIT)
    };
    int currentPoint;
...
GameState RunMenu(Menu * menu, Menu::MenuKey key); //принимает структуру меню указатель на перечисление MenuKey
